...
Let a seller list and view only the orders/line items containing their own products, scoped and financially redacted at the database level, and drive their own per-seller fulfillment record through a validated, audited, conflict-safe state machine created automatically on payment approval — with realtime updates and no direct client writes.
