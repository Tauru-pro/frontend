...
Provide the cattle breed catalog for the Tauru marketplace via Supabase (Postgres with RLS): public read access for the storefront and seller bull-listing flows, and write access (create/edit/delete) restricted to `SUPER_ADMIN` — replacing the previous legacy REST backend for this domain.
