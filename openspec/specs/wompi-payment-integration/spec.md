...
Integrate the Wompi Widget as the marketplace's payment provider: generate integrity signatures and open the widget entirely server-side/via Edge Function (never exposing secrets to the frontend), validate and idempotently process Wompi webhooks with amount/reference checks, enforce a forward-only, idempotent payment status state machine, reconcile stuck payments against the Wompi transactions API, and keep sandbox and production credentials fully separated.
