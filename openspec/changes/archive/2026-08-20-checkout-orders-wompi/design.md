...
**Bug found under genuine concurrency (task 13.1/13.2), fixed in migration `0041`**: `create_order_with_items`'s idempotency check was "SELECT existing → INSERT if not found." Two truly-simultaneous requests (real double-click, two tabs — not just two sequential calls) can both pass the SELECT before either commits, so the losing request hit the `UNIQUE(user_id, idempotency_key)` constraint as a raw, uncaught `unique_violation`, surfacing as a bare `500 INTERNAL_ERROR` instead of gracefully returning the winner's order. The constraint itself worked correctly throughout — data integrity was never at risk, exactly one order was ever created — this was purely an unhandled-exception/UX bug. Fixed by wrapping the `orders` INSERT in its own `BEGIN/EXCEPTION WHEN unique_violation` block that re-runs the idempotency lookup and returns the winner's row, identical in shape to the pre-existing "already exists" path. Verified with 3 genuinely concurrent requests post-fix: all three returned the identical order.
