...
@Component({
  ...
  imports: [DataTableComponent, TableEmptyDirective, TableCellDirective, ButtonComponent, PricePipe],
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-0.5">Gestiona el pago a vendedores</p>
        </div>
        <app-button iconPath="M12 4v16m8-8H4" (clicked)="router.navigate(['/admin/settlements/new'])">
          Nueva liquidación
        </app-button>
      </div>
      ...
      >
        ...
        <ng-template tableCell="actions" let-item>
          <button
            (click)="router.navigate(['/admin/settlements', item.id])"
        ...
        <ng-template tableEmpty>
          <div class="py-16 flex flex-col items-center text-center px-6">
            <p class="text-gray-800 font-semibold mb-1">Sin liquidaciones registradas</p>
            <p class="text-gray-400 text-sm mb-5">Crea la primera liquidación para un vendedor.</p>
            <app-button (clicked)="router.navigate(['/admin/settlements/new'])">
              Nueva liquidación
            </app-button>
          </div>
        </ng-template>
...
export default class SettlementsComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      this.errorMsg.set('No se pudieron cargar las liquidaciones. Intenta de nuevo.');
    } finally {
